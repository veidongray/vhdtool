...
/// @brief Disk Type Name value start 0
...
int Read_Binary(char* File_Name)
{
	...
	tempSize = size / 16;
	printf("<----------------------------------->\n");
	for (addr = 0, addrIndex -= 0; addrIndex < tempSize; addr += 16, addrIndex++)
	{
		...
		if (count)
		{
			showFlag = 1;///If next count value is zero then show
			printf("%.8lX: ", addr);
	...
	if (size % 16)
	{
		...
		for (i = 0; i < 16 - (tempSize % 16); i++)
		{
			printf(" ");
		}
		printf("| ");

		for (i = addr - (size % 16); i < tempSize; i++)
		{
			if (buff[i] < '~' && buff[i]>' ')
			{
				printf("%c", buff[i]);
